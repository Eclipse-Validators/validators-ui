...
export const EthTransfer: React.FC<EthTransferProps> = ({
  destinationAddress,
  isValidAddress,
}) => {
  const [solAmount, setSolAmount] = useState("");
  const { solBalance, isTransferDisabled, handleSolTransfer } =
    useSolTransfer();
  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      <p>
        ETH Balance:{" "}
        {solBalance !== null ? solBalance.toFixed(4) : "Loading..."} ETH
      </p>
      <Input
        type="number"
        value={solAmount}
