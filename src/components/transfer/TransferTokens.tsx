...
import { createTransferInstruction, TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID, getAssociatedTokenAddressSync, createAssociatedTokenAccountInstruction, createCloseAccountInstruction } from '@solana/spl-token'
    ...
    >
        <CardHeader>
            <CardTitle className="flex items-center justify-between">
                <span>{token.metadata?.name || token.mint.slice(0, 4) + '...' + token.mint.slice(-4)}</span>
                <Checkbox
                    checked={isSelected}
        ...
        <CardContent>
            <div className="flex items-center space-x-4">
                {token.metadata?.image && <img src={token.metadata.image} alt={token.metadata.name} className="w-16 h-16 rounded" />}
                <div>
...
const TransferTokens: React.FC = () => {
    ...
    const { tokens: token2022Tokens, loading: loading2022, error: error2022, refreshTokens: refresh2022Tokens } = useWalletTokens(true)
    const { tokens: splTokens, loading: loadingSPL, error: errorSPL, refreshTokens: refreshSPLTokens } = useSPLTokens(true)
    ...
    const handleTokenSelection = (tokenAccount: string) => {
        ...
        const token = [...token2022Tokens, ...splTokens].find(t => t.tokenAccount === tokenAccount)
    ...
    const handleTransfer = async () => {
        ...
        try {
            ...
            for (const tokenAccount of selectedTokens) {
                const programAddress = activeTab === 'spl' ? TOKEN_PROGRAM_ID : TOKEN_2022_PROGRAM_ID;
                const token = tokensToTransfer.find(t => t.tokenAccount === tokenAccount)
                if (!token) continue

                const amount = token.decimals === 0 ? 1 : (parseFloat(tokenAmounts[tokenAccount]) || token.amount)
                if (amount > token.amount) {
                    toast.error(`Insufficient balance for token ${token.metadata?.symbol || token.mint}`)
                    continue
                }

                const destinationTokenAccount = getAssociatedTokenAddressSync(new PublicKey(token.mint), new PublicKey(destinationAddress), true, programAddress);
                let addCreateTokenAccountIx = false;
                try {
                    const accountExists = await connection.getAccountInfo(destinationTokenAccount);
                    if (!accountExists || !accountExists.data || accountExists.data.length === 0) {
                ...
                if (addCreateTokenAccountIx) {
                    currentTx.add(createAssociatedTokenAccountInstruction(
                        publicKey,
                ...
                if (amount === 1 && token.decimals === 0 || amount === token.amount) {
                    const tokenCloseInstruction = createCloseAccountInstruction(new PublicKey(token.tokenAccount), publicKey, publicKey, undefined, programAddress);
            ...
            const confirmations = await Promise.all(signatures.map(signature =>
                connection.confirmTransaction(signature)
            ))
            if (confirmations.some(confirmation => confirmation.value.err)) {
                logger.error('Some transfers failed, try again!', {
                    signatures: signatures,
                    confirmations: confirmations.map(confirmation => confirmation.value.err)
                });
                toast.error('Some transfers failed, try again!');
            }
            else {
                toast.success(`Successfully transferred ${selectedTokens.length} tokens`, {
                    description: 'You can now view your transactions on the Solana blockchain',
                    action: {
                        label: 'View Transactions',
                        onClick: () => window.open(`https://${process.env.NEXT_PUBLIC_EXPLORER!}/tx/${signatures[0]}`, '_blank')
                    }
                })
            }
        } catch (error) {
    ...
    const renderTokenCards = useCallback((tokens: FetchedTokenInfo[]) => {
        ...
        <div className="container mx-auto p-4">
            ...
            <div className="flex flex-col items-center space-y-4">
                ...
                {!isValidAddress && destinationAddress && (
                    <p className="text-red-500 text-sm">Please enter a Solana compatible address</p>
                )}
                <Button
                    className='w-96'
                    variant='outline'
                ...
                >
                    {selectedTokens.length > 0 ? `Transfer ${selectedTokens.length} tokens` : 'Select tokens to transfer'}
                </Button>
            </div>
