...
const NFTCardFull: React.FC<NFTFullViewProps> = ({ nft }) => {
    ...
    const handleTransfer = async () => {
        if (!wallet.publicKey || !wallet.signTransaction || !isValidAddress || !isOwner) return;
        ...
        try {
            ...
            toast.success('NFT transferred successfully', {
                description: 'You can now view your transaction on the Solana blockchain',
                action: {
                    label: 'View Transaction',
                    onClick: () => window.open(`${process.env.NEXT_PUBLIC_EXPLORER}/tx/${signature}`, '_blank'),
                },
            });
            setIsOwner(false);
                    ...
                    <CardTitle className="flex justify-between items-center">
                        ...
                        {isOwner && (
                            <Dialog open={isTransferModalOpen} onOpenChange={setIsTransferModalOpen}>
                                ...
                                <DialogContent>
                                    <DialogHeader>
                                        <DialogTitle>Transfer {nft.metadata?.name || 'NFT'}</DialogTitle>
                                        <DialogDescription>
                                    ...
                                    <Input
                                        ...
                                        className={isValidAddress ? 'border-green-500' : 'border-red-500'}
                                    />
                ...
                <CardContent>
                    <div className="flex flex-col md:flex-row">
                        ...
                        <div className="w-full md:w-1/2 md:pl-4 mt-4 md:mt-0">
                            <p><strong>Symbol:</strong> {nft.metadata?.symbol || 'N/A'}</p>
                            <p><strong>Mint:</strong> <CopyableText text={nft.mint} maxLength={20} /></p>
                            <p><strong>Owner:</strong> <CopyableText text={nft.owner} maxLength={20} /></p>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {nft.metadata.attributes.map((attr, index) => (
                                            <div key={index} className="bg-secondary p-2 rounded">
                                                <p className="font-bold text-background">{attr.trait_type}</p>
