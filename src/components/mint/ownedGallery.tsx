...
export function OwnedNFTGallery({ ownedNftsData, setOwnedNftsData }: { ownedNftsData: NFTData[], setOwnedNftsData: React.Dispatch<React.SetStateAction<NFTData[]>> }) {
    ...
    const fetchOwnedNFTs = useCallback(async () => {
        ...
        remainingTokens.current = groupTokens.filter(tokenMint => !fetchedTokens.current.has(tokenMint))
        ...
        setOwnedNftsData(prev => {
            const uniqueNfts = [...prev, ...newOwnedNftsData].filter((nft, index, self) =>
        ...
        setLoading(false)
    }, [fetchNFTChunk, hashlist, tokens, membersLoading, tokensLoading, setOwnedNftsData])
    ...
    const loadMore = useCallback(async () => {
        ...
        setOwnedNftsData(prev => {
            const uniqueNfts = [...prev, ...newNftsData].filter((nft, index, self) =>
                index === self.findIndex((t) => t.address === nft.address)
            )
            return uniqueNfts
        })
